...
def jmp_fit_and_plots(df: pd.DataFrame, y, effects, *,
    ...
    def _yhat_resid(yi, model):
        ...
        X = df[effects]; y_true = pd.to_numeric(df[yi], errors="coerce")
        y_hat = pd.Series(model.predict(X), index=X.index); m = y_true.notna() & np.isfinite(y_hat)
    ...
    elif isinstance(regressor, str): chosen_regs = [regressor]
    else: chosen_regs = list(regressor)
    for yi in ys:
        model = models[yi]
        y_true, y_hat, resid = _yhat_resid(yi, model)
        figs_reg = []
        for xr in chosen_regs:
            x = pd.to_numeric(df[xr], errors="coerce"); m = x.notna() & y_true.notna()
            x_sc, y_sc = x[m].to_numpy(), y_true[m].to_numpy()
            fig = plt.figure(); ax = plt.gca()
            if x_sc.size:
                ax.scatter(x_sc, y_sc, s=25)
                grid, y_line = _line(model, xr, x_sc.min(), x_sc.max())
        ...
        if len(y_hat):
            mn, mx = float(min(y_hat.min(), y_true.min())), float(max(y_hat.max(), y_true.max()))
            ax.plot([mn, mx], [mn, mx], linewidth=1); ax.axhline(float(y_true.mean()), linewidth=1)
        ax.set_xlabel(f"{yi} Predicted"); ax.set_ylabel(f"{yi} Actual"); ax.set_title("Actual by Predicted Plot")
        plt.tight_layout(); plt.show()
        fig_resid = plt.figure(); ax = plt.gca()
        ax.scatter(y_hat.to_numpy(), resid.to_numpy(), s=25); ax.axhline(0.0, linewidth=1)
        ax.set_xlabel(f"{yi} Predicted"); ax.set_ylabel(f"{yi} Residual"); ax.set_title("Residual by Predicted Plot")
        plt.tight_layout(); plt.show()
        results[yi] = {"model": model, "fig_regression": figs_reg, "fig_actual": fig_abp, "fig_residual": fig_resid}
    return results
